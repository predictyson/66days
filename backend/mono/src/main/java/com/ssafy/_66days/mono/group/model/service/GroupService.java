...
import com.ssafy._66days.mono.group.model.dto.GroupAchievementDetailResponseDTO;
...
public class GroupService {
	...
	public List<UserManageDTO> getGroupMembers(Long groupId) {
		List<GroupMember> groupMemberList = groupMemberRepository.findByGroupAndAuthorityNot(
				groupRepository.findById(groupId), DROP);
		List<UserManageDTO> userManageDTOList = groupMemberList.stream()
				.map(groupMember -> UserManageDTO.of(userRepository.findById(groupMember.getUser().getUserId())
	...
	public List<UserManageDTO> getGroupApplyList(Long groupId) {
		Group group = groupRepository.findById(groupId)
				.orElseThrow(() -> new NoSuchElementException("group doesn't exist"));
		...
		List<UserManageDTO> userManageDTOList = applyList.stream().map(apply ->
				UserManageDTO.of(userRepository.findById(apply.getUser().getUserId())
						.orElseThrow(() -> new NoSuchElementException("user doesn't exist")))
	...
	public void setGroupMemberState(Long groupId, String state, String userName) throws InputMismatchException {
		...
		if (!(state.equals(MANAGER) || state.equals(MEMBER) || state.equals(DROP)))
			throw new InputMismatchException("권한 설정이 잘못 입력되었습니다");
		User user = userRepository.findByNickname(userName)
				.orElseThrow(() -> new NoSuchElementException("user doesn't exist"));
		Group group = groupRepository.findById(groupId)
				.orElseThrow(() -> new NoSuchElementException("group doesn't exist"));
		GroupMember groupMember = groupMemberRepository.findByGroupAndUser(group, user)
				.orElseThrow(() -> new NoSuchElementException("user is not in a group"));
		if (state.equals(MANAGER)) {
			// 매니저 인원 수: 최대 3명
			Long managerSize = groupMemberRepository.countByGroupAndAuthorityAndIsDeleted(group, MANAGER, false);
			if (managerSize >= 3L)
				throw new InputMismatchException("설정 가능한 매니저 수가 초과했습니다");
	...
	public void setGroupApplyState(Long groupId, String state, String userName) throws Exception {
		...
		if (!(state.equals(ACCEPTED) || state.equals(REJECTED)))
			throw new InputMismatchException("그룹 가입승인 설정이 잘못 입력되었습니다");
		User user = userRepository.findByNickname(userName)
				.orElseThrow(() -> new NoSuchElementException("user doesn't exist"));
		Group group = groupRepository.findById(groupId)
				.orElseThrow(() -> new NoSuchElementException("group doesn't exist"));
		GroupApply groupApply = groupApplyRepository.findByUserAndGroup(user, group)
				.orElseThrow(() -> new NoSuchElementException("groupApply doesn't exist"));
		if (state.equals(ACCEPTED)) {
			// 가입한 그룹 수가 5개 이하인지 확인
			Long groupSize = groupMemberRepository.countByUserAndIsDeleted(user, false);
			if (groupSize > 5L)
				throw new InputMismatchException("사용자의 가입한 그룹 수가 한도 초과했습니다");
		}
	...
	public void applyGroup(Long groupId, String state, UUID userId) {
		state = state.toUpperCase();
		if (!(state.equals(APPLIED) || state.equals(CANCELLED)))
			throw new InputMismatchException("그룹 가입신청이 잘못 입력되었습니다");
	...
	public List<GroupAchievementResponseDTO> getGroupAchievement(UUID userId, Long groupId) {
		...
		GroupMember groupMember = groupMemberRepository.findByGroupAndUserAndIsDeleted(group, user,
						false)  // 그룹멤버 객체 받아오기
		...
		for (int i = 0; i
				< challenges.size(); i++) {                                                           // 챌린지 id들을 temp에 담는다
			temp.add(challenges.get(i).getChallengeId());
		}

		List<GroupAchievement> groupAchievements = groupAchievementRepository.findByGroup(
				group);               // 그룹의 업적 객체 받아오기
		List<GroupAchievementResponseDTO> GroupAchievementResponseDTOs = new ArrayList<>();                     // 업적 정보 저장할 빈 배열
		if (groupAchievements
				!= null) {                                                                        // 그룹에 업적이 있다면
			for (int i = 0;
			     i < groupAchievements.size(); i++) {                                                // 업적 정보를 DTO로 변환하여
				GroupAchievement tempGroupAchievement = groupAchievements.get(i);
				GroupAchievementResponseDTO groupAchievementResponseDTO = GroupAchievementResponseDTO.of(
						tempGroupAchievement);
				GroupAchievementResponseDTOs.add(
						groupAchievementResponseDTO);                                  // 배열에 담는다
				temp.remove(tempGroupAchievement.getChallenge()
						.getChallengeId());                              // 나온 업적들은 temp에서 하나씩 지운다
			}
		}
		for (int j = 0; j
				< temp.size(); j++) {                                                                 // temp에 남은 challengeId = 그룹에게 하나도 없는 업적
			Long challengeId = temp.get(j);
			Challenge challenge = challengeRepository.findById(
							challengeId)                                     // 챌린지 객체를 가져와서
					.orElseThrow(() -> new IllegalArgumentException("존재하지 않는 챌린지입니다"));
			GroupAchievementResponseDTO groupAchievementResponseDTO = GroupAchievementResponseDTO.from(
					challenge);  // 객체로 만든다 이때 업적 갯수 0으로
			GroupAchievementResponseDTOs.add(
					groupAchievementResponseDTO);                                      // 반환 list에 추가한다
		...
		for (String state : stateList) {
			...
			if (groupChallenges != null) {
				for (int i = 0; i < groupChallenges.size(); i++) {
					...
					GroupAchievementDetailResponseDTO achievementDetailDTO = GroupAchievementDetailResponseDTO.of(
							groupChallenge);
